...
from generate_sample_timetable import create_pdf, load_grids
from config import (EARLY_STOPPING_ROUNDS, ENABLE_PLOTS, ENABLE_VERBOSE,
                    IMPROVEMENT_THRESHOLD, ITERATIONS, PLOT_DPI,
...
def run(ds, name, opts, seed, run_number):
    random.seed(seed)

    LOGGER.info("Run %d/%d", run_number, RUNS)
...
def plot(summary, hist, runs):
    ...
    curve = complete.groupby("Iteration")["BestFitness"].mean(); axis.plot(curve.index, curve.values, color="#A61C1C", linewidth=2.5, label=f"Mean of {RUNS} runs")
...
def reports(ds,tt,perf,summary):
 ...
 pd.DataFrame(detail.items(),columns=["Constraint","Violations"]).to_csv(RESULTS/"constraint_violation_report.csv",index=False);pd.DataFrame(detail.items(),columns=["Constraint","Violations"]).to_csv(RESULTS/"Constraint_Report.csv",index=False);summary.to_csv(RESULTS/"optimization_summary_report.csv",index=False)
 try:
  from reportlab.lib.pagesizes import A4,landscape
  from reportlab.lib import colors
  from reportlab.platypus import SimpleDocTemplate,Paragraph,Spacer,Table,TableStyle
  from reportlab.lib.styles import getSampleStyleSheet
  style=getSampleStyleSheet(); data=[list(summary.columns)]+summary.round(3).astype(str).values.tolist();tab=Table(data,repeatRows=1);tab.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),colors.HexColor("#17365D")),("TEXTCOLOR",(0,0),(-1,0),colors.white),("GRID",(0,0),(-1,-1),.3,colors.grey),("FONTSIZE",(0,0),(-1,-1),7)]));SimpleDocTemplate(str(RESULTS/"Experimental_Results_Report.pdf"),pagesize=landscape(A4)).build([Paragraph("Experimental Results Report",style["Title"]),Paragraph(f"Adaptive TLBO: {RUNS} independently seeded runs per configuration.",style["BodyText"]),Spacer(1,10),tab])
 except ImportError: pass
def main():
 configure_logging(ENABLE_VERBOSE)
 RESULTS.mkdir(exist_ok=True);clean_results();ds=Dataset();ds.load_data();rows=[];hs=[];best=None
 for i,(n,opt) in enumerate(EXPERIMENTS):
  LOGGER.info("%s", "=" * 50); LOGGER.info("Experiment %d/%d | %s", i+1, len(EXPERIMENTS), n); LOGGER.info("%s", "=" * 50)
  for r in range(RUNS):
   tt,h,row=run(ds,n,opt,RANDOM_SEED+i*RUNS+r,r+1);rows.append(row);h.insert(0,"Run",r+1);h.insert(0,"Seed",row["Seed"]);h.insert(0,"Experiment",n);hs.append(h)
   if n=="Complete Adaptive TLBO" and (best is None or row["Fitness"]>best[2]["Fitness"]):best=(tt,h,row)
  LOGGER.info("Completed %s", n)
 ...
 create_pdf(load_grids(), RESULTS / "University_Timetable.pdf")
 for duplicate_name in ("faculty_workload_report.csv", "classroom_utilization_report.csv", "constraint_violation_report.csv", "final_timetable.csv"):
  duplicate = RESULTS / duplicate_name
  if duplicate.exists():
   duplicate.unlink()
 LOGGER.info("Completed all experiments successfully: %d total runs.", len(rows))
