...
class ExcelFormatter:
    ...
    def _section_timetable_sheet(self, wb, section: str, timetable) -> None:
        ...
        for day in self.days:
            ...
            for column in columns[1:]:
                if str(column).startswith("Lunch Break"):
    ...
    def _style_timetable(self, ws, title_columns: int, merge_lunch: bool) -> None:
        ...
        lunch_col = next((column for column in range(1, ws.max_column + 1) if str(ws.cell(row=2, column=column).value).startswith("Lunch Break")), 0)
    ...
    def _section_columns(self) -> List[str]:
        ...
        for period in self.periods:
            if period == 5:
                columns.append("Lunch Break (1 PM - 2 PM)")
